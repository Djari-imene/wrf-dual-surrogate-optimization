#!/usr/bin/env python3
"""
Train a CatBoost surrogate model using FLAML on the target-encoded dataset,
...
os.environ.setdefault("CATBOOST_ALLOW_SLOW_TRAIN", "1")
...
TIME_BUDGET = 300              
N_FOLDS     = 10
DATASET     = "dataset_encoded_target.csv"
OUTPUT_PATH = "catboost_surrogate_target.pkl"

# ── CatBoost sklearn baseline (choose-better fallback) ────────────────────────
...
def cv_rmse(estimator, X: np.ndarray, y: np.ndarray) -> float:
    """Quick 5-fold CV RMSE used for choose-better comparison."""
...
def main():
    print("=" * 62)
    print("CATBOOST SURROGATE TRAINING  —  FLAML HPO + Choose-Better")
    ...
    # 2. Baseline CatBoost CV RMSE (choose-better reference) ──────────────────
    print(f"\n Step 1 — Baseline CatBoost (500 trees, lr=0.05, depth=6) ...")
    with suppress_stderr():
        baseline_rmse = cv_rmse(CATBOOST_BASELINE, X, y)
    print(f"   Baseline CV RMSE : {baseline_rmse:.4f}")

    # 3. FLAML HPO — restricted to CatBoost ───────────────────────────────────
    print(f"\n Step 2: FLAML HPO (budget={TIME_BUDGET}s, metric=rmse, estimator=catboost) ...")
    automl = AutoML()
    with suppress_stderr():
        automl.fit(
            X_train        = X,
    ...
    # Ensure CatBoost stays silent after FLAML
    if hasattr(flaml_surrogate, "set_params"):
        try:
            flaml_surrogate.set_params(verbose=0, allow_writing_files=False)
        except Exception:
            pass
    ...
    # Note: CatBoost typically has higher train-test R² gap than RF because
    # it fits training data more tightly (gradient boosting). A gap up to 0.15
    # is normal and does not indicate harmful overfitting if test metrics are stable.

    # 8. Save per-fold metrics ─────────────────────────────────────────────────
    cv_df.to_csv("catboost_cv_metrics.csv")
    print(f"\n  ✓ Per-fold metrics saved -> catboost_cv_metrics.csv")
    ...
    print_metrics("FINAL CATBOOST MODEL (full-data refit)", final_metrics)

    # 10. Save surrogate + metadata ────────────────────────────────────────────
    # Compute relative RMSE for reporting
    ...
    print(f"\n{'='*62}")
    print(f" CatBoost surrogate saved  ->  {OUTPUT_PATH}")
